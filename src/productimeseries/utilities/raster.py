...
from src.productimeseries.exceptions import NoSentinelException
from src.productimeseries.execution_mode import ExecutionMode
from src.productimeseries.minio import MinioConnection
from src.productimeseries.rasterpoint import RasterPoint
from src.productimeseries.utilities.geometries import (
    _convert_3D_2D,
    _project_shape,
)
...
def _download_sample_band_by_title(
    ...
    raise NoSentinelException(
        f"Either data of product {title} wasn't found in MinIO or 10m bands weren't found for that product.")
...
def _rescale_band(
    ...
    if img_resolution != spatial_resol:
        ...
        new_kwargs["transform"] = rasterio.Affine(
            spatial_resol, kwargs["transform"][1], kwargs["transform"][2], kwargs["transform"][3], -spatial_resol,
            kwargs["transform"][5])
